"""
Post-processing znieksztalcenia depth
====================================
Naklada realistyczne znieksztalcenia kamery depth (szum, wyrownanie, projekcja)
na obrazy depth_ground_truth przy uzyciu synthetic-rgbd-camera-model.

Wyjscie "depth_perfect" pozostaje w ukladzie kamery depth, aby nie dodawac
dziur po okluzjach z widoku RGB do idealnej referencji glebokosci.

Uruchamiaj ten skrypt po generate_data.py, najlepiej w venv z open3d.
"""
...
def resolve_camera_model_paths(main_dir: str) -> tuple[str, str]:
    """Wyszukuje katalog modelu kamery oraz plik parametrow.
    ...
    # Najpierw probuj aktualny, pojedynczy katalog repozytorium.
    ...
    raise FileNotFoundError(
        "Nie znaleziono plikow modelu kamery. Oczekiwano "
        f"{base_dir}/(params/femto_mega.json, src/) lub "
        f"{base_dir}/synthetic-rgbd-camera-model/(params/femto_mega.json, src/)."
    )


def add_camera_model_to_path(camera_model_root: str) -> str:
    """Dodaje katalog modelu kamery do sys.path i zwraca jego sciezke."""
...
def fill_missing_depth_pixels(depth_img):
    """Uzupelnia dziury (zera) w obrazie depth najblizszymi poprawnymi pikselami."""
...
def collect_sample_jobs(root_dir: str) -> list[SampleJob]:
    """Zbiera wszystkie pary depth/RGB z katalogow run."""
    jobs = []
...
def build_worker_manifests(sample_jobs: list[SampleJob], workers: int, manifest_dir: str) -> list[str]:
    """Dzieli probki na workerow i zapisuje osobny manifest tekstowy dla kazdego."""
...
def process_worker_manifest(manifest_path: str, camera_model_root: str, params_path: str, global_processed_dir: str):
    """Przetwarza probki przypisane do jednego workera."""
...
def find_run_dirs(root_dir: str) -> list:
    """Wyszukuje rekurencyjnie katalogi run zawierajace folder raw/."""
    runs = []
...
def main():
    """Wczytuje konfiguracje i uruchamia przetwarzanie depth w trybie sekwencyjnym lub rownoleglym."""
