...
class ProjectionManager:
    ...
    def _depth_image_to_point_cloud_with_K(
        self, depth_img, K, dist_coeffs, projection_model="equisolid"
    ):
        """Back-project a depth map to a 3D point cloud.

        Supports two projection models:
        - "equisolid": Fisheye equisolid (r = 2f·sin(θ/2)), matching Blender's
          FISHEYE_EQUISOLID panoramic camera. Requires depth_params with 'px'
          (pixel size in mm) to be set on the ProjectionManager instance.
        - "pinhole": Classic pinhole + OpenCV distortion (Brown-Conrady /
          rational polynomial). Uses cv2.undistortPoints for backward compat.

        Args:
            depth_img (np.ndarray): 2D depth image in raw units (mm as uint16).
            K (np.ndarray): 3x3 camera intrinsic matrix (fx, fy, cx, cy in pixels).
            dist_coeffs (array-like): Distortion coefficients.
                For pinhole: OpenCV coefficients (k1..k6, p1, p2).
                For equisolid: ignored (projection is analytical).
            projection_model (str): "equisolid" or "pinhole".

        Returns:
            np.ndarray: Nx3 array of 3D points in the camera frame (meters).
        """
        H, W = depth_img.shape
        ...
        u_valid = u[valid]
        v_valid = v[valid]
        Z = depth_m[valid]  # ray distance from camera origin

        if projection_model == "equisolid":
            points = self._equisolid_backproject(u_valid, v_valid, Z, K)
        elif projection_model == "pinhole":
            uv = np.stack([u_valid, v_valid], axis=1)
            ...
            points = np.stack([X, Y, Z_cam], axis=1)
        else:
            raise ValueError(
                f"Unknown projection_model '{projection_model}'. "
                "Use 'equisolid' or 'pinhole'."
            )

        return points

    def _equisolid_backproject(self, u, v, ray_distance, K):
        """Back-project pixels through a fisheye equisolid model.

        The equisolid (equal-area) projection maps incidence angle θ to
        sensor-plane radius r via:  r = 2·f·sin(θ/2)

        Inverse:  θ = 2·arcsin(r / (2·f))

        The depth value from Blender's fisheye camera is the Euclidean
        distance along the ray (ray distance), NOT the Z-component.

        Args:
            u (np.ndarray): Pixel x-coordinates (1D, float or int).
            v (np.ndarray): Pixel y-coordinates (1D, float or int).
            ray_distance (np.ndarray): Distance from camera origin along
                the ray for each pixel, in meters.
            K (np.ndarray): 3x3 intrinsic matrix. fx, fy are in pixels;
                self.depth_params['px'] provides mm/pixel conversion.

        Returns:
            np.ndarray: Nx3 array of 3D points (X, Y, Z) in camera frame.
        """
        cx = K[0, 2]
        cy = K[1, 2]
        px = self.depth_params["px"]  # mm per pixel
        fx_mm = K[0, 0] * px          # focal length in mm

        # Pixel offset from principal point → sensor-plane position in mm
        x_sensor = (u - cx) * px
        y_sensor = (v - cy) * px

        # Radial distance on sensor plane (mm)
        r = np.sqrt(x_sensor**2 + y_sensor**2)

        # Inverse equisolid: θ = 2·arcsin(r / (2·f))
        ...
        # 3D ray direction (unit vector)
        sin_theta = np.sin(theta)
        ray_x = sin_theta * np.cos(phi)
        ray_y = sin_theta * np.sin(phi)
        ray_z = np.cos(theta)

        # Scale by ray distance to get 3D point
        X = ray_x * ray_distance
        Y = ray_y * ray_distance
        Z = ray_z * ray_distance

        return np.stack([X, Y, Z], axis=1)

    def _transform_point_cloud_to_rgb(self, point_cloud, T):
        """Apply a 4x4 transform to bring points into RGB camera frame.

        Args:
            point_cloud (np.ndarray): Nx3 array of points in depth frame.
            T (np.ndarray): 4x4 homogeneous transformation matrix.

        Returns:
            np.ndarray: Nx3 array of points in RGB camera frame.
        """
    ...
    def _project_points_to_pixels_filtered(
        ...
        Args:
            ...
            points_rgb (np.ndarray): Nx3 points in RGB camera frame.
            K_rgb (np.ndarray): 3x3 intrinsics of RGB camera.
            image_shape (tuple): (height, width) of the image.

        Returns:
            tuple:
                - proj_pixels (np.ndarray): Mx2 float pixel coordinates.
                - filtered_depths (np.ndarray): M depth values.
                - indices (list[int]): Indices of kept points.
                - colors (np.ndarray): Mx3 RGB colors normalized to [0,1].
        """
        H, W = image_shape
        ...
        print("→ points before filtering:", u.size)
        ...
        print("points in view:", inside.sum(), "out of", len(u2i))
    ...
    def _get_projected_depth_image(
        ...
        Args:
            points_rgb (np.ndarray): Mx3 points in RGB camera frame (meters).
            K_rgb (np.ndarray): 3x3 intrinsics of RGB camera.
        ...
        Returns:
            np.ndarray: HxW uint16 depth image (0 = no data).
        """
        H, W = image_shape
    ...
    def get_aligned_depth_img(self, depth_img: np.ndarray, rgb_img: np.ndarray):
        ...
        Args:
            ...
            rgb_img (np.ndarray): Corresponding RGB image (HxWx3).

        Returns:
            np.ndarray: Aligned uint16 depth image in RGB resolution.
        """
        point_cloud_depth = self._depth_image_to_point_cloud_with_K(
            depth_img, self.K_depth, dist_coeffs=self.depth_params["dist"]
        )
        print("Depth point cloud shape:", point_cloud_depth.shape)
        pc_rgb_via_inv = self._transform_point_cloud_to_rgb(point_cloud_depth, self.T)

        proj1, filt_depth1, idx, colors = self._project_points_to_pixels_filtered(
    ...
    def get_aligned_depth_img_no_interp(
        ...
        point_cloud_rgb = self._transform_point_cloud_to_rgb(point_cloud_depth, self.T)
